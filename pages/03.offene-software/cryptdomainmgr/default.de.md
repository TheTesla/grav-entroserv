...
Wir benötigen im Beispiel den Eintrag ``"v=spf1 +mx ?all"`` als ``TXT``-Record unter ``entroserv.de``. Das ``+mx`` bedeutet, unser MX-Record zeigt den Server, der auch E-Mails für ``entroserv.de``versenden darf. Cryptdomainmgr benötigt den Eintrag:
