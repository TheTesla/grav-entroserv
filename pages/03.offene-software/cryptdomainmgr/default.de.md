...
Einige wichtige DNS-Records wurden bisher noch nicht besprochen. Im vorangegangenen Beispiel hatten wir eine von der Serverdomain getrennte E-Mail-Domain. Diese werden über den _MX_-Record verknüpft. Unter ``èntroserv.de`` muss in unserem Beispiel ein DNS-Eintrag vom Typ ``MX`` angelegt werden, welcher auf ``pserver.entroserv.de`` zeigt. Dieser Eintrag wird für den Empfang von E-Mails benötigt. Der sendende Server will eine E-Mail an _info@entroserv.de_ schicken. Er schaut auf _entroserv.de_ nach, was im MX-Record für ein Server eingetragen ist. Efr weiß nun, dass er die E-Mail an den Server _pserver.entroserv.de_ übertragen muss. 
